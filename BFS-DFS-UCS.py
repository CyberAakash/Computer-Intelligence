...
class Graph:
    ...
    def delete_edge(self, src, dest):
        if src in self.vertices and dest in self.vertices:
            # Deleting edge from src to dest
            ...
            # Deleting edge from dest to src
            current_node = self.vertices[dest]
            prev_node = None
            while current_node:
                if current_node.data == src:
                    if prev_node is None:
                        self.vertices[dest] = current_node.next
                    else:
                        prev_node.next = current_node.next
                    break
                prev_node = current_node
...
def main():
    ...
    while True:
        ...
        ch = int(input())
        if ch == 1:
            print("Enter the data for the vertex:")
            data = input()
